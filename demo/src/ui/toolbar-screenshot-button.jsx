...
export default function ToolbarScreenshotButton({ mode }) {
  ...
  let imageBrowserDownload = imageUri => {
    ...
    filename = window.prompt('Write filename', filename);
    if (!filename) return;
  ...
  if ([MODE_3D_FIRST_PERSON, MODE_3D_VIEW].includes(mode)) {
    return (
      <ToolbarButton active={false} tooltip={'Get Screenshot'} onClick={saveScreenshotToFile}>
        <FaCamera />
      </ToolbarButton>
    );
  }
  ...
  if ([MODE_IDLE,
    ...
    return (
      <ToolbarButton active={false} tooltip={'Get Screenshot'} onClick={saveSVGScreenshotToFile}>
        <FaCamera />
      </ToolbarButton>
    );
  }
